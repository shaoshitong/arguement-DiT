...
def get_random_captions_from_indices(y, data_path='/data/shared_data/ILSVRC2012/train',prompt_root='/data/shared_data/ILSVRC2012/caption_embeddings'):
    n = len(y)  # batch_size
    ...
    for i in range(n):
        ...
        embedding_file = os.path.join(prompt_root, cls + ".pt")
        # 构建captions.csv的路径
...
def main(args):
    ...
    folder_name = f"0405--{model_string_name}-{ckpt_string_name}-size-{args.image_size}-vae-{args.vae}-" \
    ...
    for _ in pbar:
        ...
        embedding_list = [emb.squeeze(0) for emb in embedding_list]  # 去掉第一个维度，变为 [32, 2048]
        text_embedding = torch.stack(embedding_list, dim=0)  # 堆叠成 [32, 32, 2048]
        text_embedding = text_embedding.to(torch.float32).to(device)  # 转换为 float32 并移动到设备
        # Setup classifier-free guidance:
        if using_cfg:
            z = torch.cat([z, z], 0)
            y_null = torch.zeros_like(text_embedding).to(device)  # 创建一个全零张量，并移动到设备
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample-dir", type=str, default="samples_wocutmix")
    ...
    parser.add_argument("--ckpt", type=str, default="/home/shaoshitong/project/argument-DiT/results/20250404_172254-DiT-L-2/checkpoints/0040000.pt",
