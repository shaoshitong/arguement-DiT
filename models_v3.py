...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y, ccls, attn_mask=None, zs=None):
        """
        Forward pass of DiT.
        ...
        for i, block in enumerate(self.blocks):
            x = block(x, c)                      # (N, T, D)
            if (i + 1) == self.encoder_depth and self.z_dims is not None and zs is not None:
                zs_tilde = [projector(x.reshape(-1, D)).reshape(N, T, -1) for projector in self.projectors]
                
        if self.z_dims is not None and zs is not None:
    ...
    def forward_with_cfg(self, x, t, y, ccls, cfg_scale, attn_mask=None):
        """
        ...
        model_out = self.forward(combined, t, y, ccls, attn_mask)
