...
def get_random_captions_from_indices(y, data_path='/data/shared_data/ILSVRC2012/train'):
    n = len(y)  # batch_size
    ...
    for i in range(n):
        ...
        class_path = Path(data_path) / cls
        
        # 构建captions.csv的路径
        caption_file = class_path / "caption_embedding.npy"
        
        embedding = np.load(caption_file, allow_pickle=True).squeeze(0)  # 去掉多余维度
...
def main(args):
    ...
    for _ in pbar:
        ...
        text_embedding = torch.tensor(np.array(embedding_list), dtype=torch.float32).to(device)


        # Setup classifier-free guidance:
        if using_cfg:
            z = torch.cat([z, z], 0)
            y_null = torch.zeros(n,text_embedding.shape[1]).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample-dir", type=str, default="samples_wocutmix_single")
    ...
    parser.add_argument("--ckpt", type=str, default="/home/shaoshitong/project/argument-DiT/results/075-DiT-L-2/checkpoints/0040000.pt",
