...
def euler_sampler(
    ...
    if cfg_scale > 1.0:
        ...
        text_null = torch.zeros_like(text_embedding).to(device=text_embedding.device)
        text_mask_null = torch.zeros_like(text_embedding_mask).bool().to(device=text_embedding_mask.device)
    ...
    with torch.no_grad():
        for i, (t_cur, t_next) in tqdm(enumerate(zip(t_steps[:-1], t_steps[1:]))):
            ...
            if cfg_scale > 1.0 and t_cur <= guidance_high and t_cur >= guidance_low:
                ...
                text_embedding_cur = torch.cat([text_embedding, text_null], dim=0)
                text_embedding_mask_cur = torch.cat([text_embedding_mask, text_mask_null], dim=0)
            else:
                model_input = x_cur
                y_cur = y
                text_embedding_cur = text_embedding
                text_embedding_mask_cur = text_embedding_mask
            kwargs = dict(ccls=y_cur, y=text_embedding_cur, attn_mask=text_embedding_mask_cur)
            # x, t, y, ccls, attn_mask=None
